...
#define UNITY_outputcontrolpoints    outputcontrolpoints
 ...
 // Define a struct to store vertex data
struct VertexData
{
    float3 position;
    float3 normal;
...
// Structured buffer to store tessellated vertices
RWStructuredBuffer<VertexData> vertices : register(u0);
RWStructuredBuffer<uint> updatedVertexCountBuffer : register(u1);
...
float FixedUnityCalcEdgeTessFactor(float3 wpos0, float3 wpos1, float edgeLen)
{
    float len = _Tess * distance(wpos0, wpos1);
    return len;
}
...
TessellationFactors UnityEdgeLengthBasedTess(float4 v0, float4 v1, float4 v2, float edgeLength)
{
    ...
    TessellationFactors tess;
    tess.edge[0] = FixedUnityCalcEdgeTessFactor(pos1, pos2, edgeLength);
    tess.edge[1] = FixedUnityCalcEdgeTessFactor(pos2, pos0, edgeLength);
    tess.edge[2] = FixedUnityCalcEdgeTessFactor(pos0, pos1, edgeLength);
    tess.inside = (tess.edge[0] + tess.edge[1] + tess.edge[2]) / 5.0f;
    return tess;
}
...
TessellationFactors patchConstantFunction(InputPatch<ControlPoint, 3> patch)
{
    ...
    // continious tesselation based on _Tess being the edge length, with camera culling
